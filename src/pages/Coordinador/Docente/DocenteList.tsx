...
const DocenteList: React.FC = () => {
  ...
  // Función para crear docentes
  const crearDocentes = async () => {
    try {
      await axiosInstance.post('/crear-docentes'); // Llama a la ruta que creará los docentes
      toast.success('Docentes creados con éxito');
      loadDocentes(); // Recargar la lista de docentes
    } catch (err) {
      const uiError = toUiError(err);
      toast.error(uiError.message || 'Error al crear los docentes');
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow-xl p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          ...
          {/* Agregar el botón para crear docentes automáticamente */}
          <button
            onClick={crearDocentes}
            className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md flex items-center justify-center transition-colors mt-4 md:mt-0"
          >
            Crear Docentes Automáticos
          </button>
        </div>
